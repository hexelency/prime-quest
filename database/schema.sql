...
create table public.mandates (
  ...
  direction public.mandate_direction not null,
  ...
  terms text,
  budget text,
  status public.mandate_status not null default 'new',
  verification_status public.verification_status not null default 'under_review',
...
create table public.matches (
  ...
  created_at timestamptz not null default now(),
  unique (buyer_mandate_id, seller_mandate_id)
);

create table public.ai_threads (
  id uuid primary key default gen_random_uuid(),
  title text not null default 'Trade intelligence session',
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);
...
-- The initial MVP accesses these tables only from server routes using Supabase's service role.
-- Add authenticated role policies after admin authentication and role checks are implemented.
