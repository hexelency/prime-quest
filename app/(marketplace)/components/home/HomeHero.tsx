...
export default function HomeHero({ offering, buyerHref }: HomeHeroProps) {
  return (
    <section className="hero" id="top">
      ...
      <div className="hero-content shell"><p className="eyebrow">Independent consultancy · Asaba, Nigeria</p><h1>{offering.heroTitle.split("\n").map((line, index) => <span key={line}>{index > 0 && <br />}<em>{index === 1 ? line : ""}</em>{index === 0 ? line : ""}</span>)}</h1><p className="hero-copy">{offering.heroCopy}</p><div className="hero-actions"><a className="button button-primary" href={buyerHref} target="_blank" rel="noreferrer">I am looking to buy <span>↗</span></a><a className="text-link" href="/mandate">I have an offering <span>Tell us about it →</span></a></div></div>
