...
type ChatMessage = { role: "user" | "assistant"; content: string; source?: "openai" | "ollama" | "local-knowledge" | "admin-context" | "mcp-shared-tools"; tool?: AiDataTool };
...
export default function AiChat() {
  ...
  const sendMessage = async (event?: FormEvent) => {
    ...
    try {
      ...
      const result = text ? JSON.parse(text) as { message?: string; error?: string; threadId?: string | null; source?: ChatMessage["source"]; tool?: AiDataTool } : {} as { message?: string; error?: string; threadId?: string | null; source?: ChatMessage["source"]; tool?: AiDataTool };
  ...
  return (
    <main className={styles.page}>
      ...
      <section className={styles.chatShell} aria-label="Trade intelligence assistant">
        <div className={styles.chatHeader}><div><strong>PrimeQuest intelligence assistant</strong><span>Internal workspace · database context enabled</span></div></div>
        <div className={styles.messages} aria-live="polite">
          ...
          {messages.map((message, index) => <div className={message.role === "user" ? styles.userMessage : styles.assistantMessage} key={`${message.role}-${index}`}><span className={styles.messageRole}>{message.role === "user" ? "You" : "PrimeQuest AI"}</span>{message.role === "assistant" && message.source && <small className={styles.sourceTag}>{message.source === "openai" ? "OPENAI" : message.source === "ollama" ? "OLLAMA" : message.source === "admin-context" || message.source === "mcp-shared-tools" ? `Live data${message.tool ? ` · ${message.tool}` : ""}` : "CHATBOT"}</small>}<p>{message.content}</p></div>)}
        ...
        <form className={styles.composer} onSubmit={sendMessage}><textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} placeholder="Ask about leads, mandates or potential matches..." rows={2} aria-label="Ask the trade intelligence assistant" /><label className={styles.model}>Agent <select value={provider} onChange={(event) => setProvider(event.target.value as Provider)} aria-label="Choose AI agent"><option value="local">CHATBOT</option><option value="openai">OPENAI</option><option value="ollama">OLLAMA</option></select></label><button type="submit" disabled={loading || !prompt.trim()} aria-label="Send message">Send <span>↗</span></button></form>
