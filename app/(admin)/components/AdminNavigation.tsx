...
const links = [
  ...
  ["05", "Lead intelligence", "/admin/leads"],
  ["06", "AI control center", "/admin/ai"],
] as const;
...
export default function AdminNavigation({ children }: Readonly<{ children: React.ReactNode }>) {
  ...
  return <div className={styles.shell}>
    <aside className={`${styles.sidebar} ${mobileOpen ? styles.sidebarOpen : ""}`} aria-label="Admin navigation">
      ...
      <nav className={styles.nav}>{links.map(([number, label, href]) => <a className={pathname === href ? styles.navItemActive : styles.navItem} href={href} key={href}><span className={styles.navIcon}>{number}</span>{label}</a>)}</nav>
